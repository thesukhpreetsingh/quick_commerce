...
function toInventoryItems(items) {
    return (items || []).map((item) => ({ productId: item.productId, quantity: item.quantity }));
}
async function handleReserveInventory(call, callback) {
...
async function handleReleaseReservation(call, callback) {
    ...
    try {
        ...
        callback(null, {
            success: true,
            items: (result.released || []).map((item) => ({ productId: item.productId, quantity: item.quantity })),
        });
    }
    catch (error) {
...
async function handleFinalizeReservation(call, callback) {
    ...
    try {
        ...
        callback(null, {
            success: true,
            items: finalizedItems.map((item) => ({ productId: item.productId, quantity: item.quantity })),
        });
    }
    catch (error) {
