...
async function getStockFromDb(productId) {
...
export async function getBulkInventory(productIds) {
...
const updateProductListLua = `
  ...
  return true
`;
export async function decreaseInventory(items) {
    ...
    try {
        ...
        for (const item of items) {
            const queryResult = await query('UPDATE products SET stock = GREATEST(stock - $1, 0) WHERE external_id = $2 RETURNING *', [item.quantity, item.productId]);
...
const reservationKey = (orderId) => `reservation:${orderId}`;
export async function reserveInventory(orderId, items) {
    ...
    const reservationEntries = items.map((item) => [String(item.productId), String(item.quantity)]);
...
export async function finalizeReservation(orderId) {
    const key = reservationKey(orderId);
    const reserved = await redisClient.hGetAll(key);
    if (!reserved || Object.keys(reserved).length === 0) {
